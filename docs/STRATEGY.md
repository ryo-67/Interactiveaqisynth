...
1. Hero: two cards with the monitor widgets' exact anatomy (2026-09-16): label, value, gauge. The AQI card holds the number under the label "AQI · now" on Live or "AQI · Oct 29" on an archive day, the number being the current AQI or the day's official AQI (D-42), over one bar in the number's colour on the six-category EPA ramp. The Breath card holds the mood word in white, following the graph's AQI line at the hour being heard, over the five-step ladder the scale and tone cards carry, its lit step in the tier's colour; the ladder is where the colour lives. Each card is a fixed width, measured from the widest thing it could show (three of the widest digit and the widest label; the widest tier word), so nothing moves between days or tiers; on laptop the pair sits centred at its own width, below laptop it spans the section and is exactly as tall as the monitor's first row. The mood sentence is gone: the gauge says what it said. No beat pulse (2026-09-15).
