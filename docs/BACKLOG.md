...
| CLN-09 | P2 | TODO | A linter, or drop the directives that pretend there is one | Measured (BUG-51): zero rules-of-hooks violations, and three unseen exhaustive-deps findings. The one worth acting on is useListenSession.ts:279, where `rest` is rebuilt every render and defeats the useMemo at 305 — the BUG-31 class. Fixing that one does not need the linter; adopting the linter is what stops the next one |
