// Cursor — the page's own pointer (CURSOR in theme.ts), drawn in the DOM so its states can move rather than snap: a ring that grows over anything clickable, and over the sky or the graph grows further while a glyph fades in at its centre — play or pause for the sky (from the sky's data-cursor, which the scene sets), the drag arrows for the graph. A CSS cursor image cannot animate, which is why this is an element that follows the pointer; the native cursor is hidden while it runs (index.css, html.has-cursor) and returns if it unmounts. Fine pointers only. Glyphs are Lucide's (icons.tsx).
...
export function Cursor() {
  ...
  const vars = { "--cur-ring": `${CURSOR.ring}px`, "--cur-pointer": `${CURSOR.pointer}px`, "--cur-glyph-ring": `${CURSOR.glyphRing}px`, "--cur-glyph": `${CURSOR.glyph}px`, "--cur-stroke": `${CURSOR.stroke}px`, "--cur-ms": `${CURSOR.ms}ms` } as React.CSSProperties;
  return (
    <div ref={ref} className="scene-cursor" data-mode={mode} aria-hidden style={vars}>
