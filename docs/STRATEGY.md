...
3. Graph panel: the day as four labelled tracks on one hour-aligned x-scale — AQI (from PM2.5, coloured by the standard EPA categories with their legend, D-23), PM2.5 µg/m³, O3 ppb, NO2 ppb — and beneath them the pulse row, which draws the engine's exact 16-step Euclidean pattern per four-hour bar (four steps under every hour column) and brightens a hit when the engine fires it. One playhead through all tracks, on the same eased hour that moves the sun, with the hour's values printed at its head. Track toggles show or hide each. PM2.5 is not drawn as a sky strip: particulate matter is the sky itself. Tapping the graph toggles play.
