...
1. Hero panel: the AQI number (display, serif, centred on the text by its own ink) at the left; at the right the mood word coloured by its AQI on the EPA ramp over the tier's sentence, always two lines. The number is the day's official AQI, or the current AQI on Live (D-42); the word follows the graph's AQI line at the hour being heard. Fit to its content, height fixed per breakpoint; no clause placeholders and no beat pulse (2026-09-15).
...
7. Monitor page (D-43): a second page of the scene, reached by two translucent icons at the foot, a horizontal swipe on phones, or the arrow keys, persisting as `?view=monitor`. The sky, the top bar, the transport and the source line stay; only the middle band slides, over 1.5 beats (a cross-fade under reduced motion), and the music never stops. A bento of frosted cards reads the engine: SCALE (PM2.5; the scale's name and a five-step ladder on EPA's lines, the lit step in the category colour), TONE (PM2.5 · NO₂; harmonicity and index as one word per tier, the same ladder), BEATS (NO₂; k / 16 and the bar's 16-step lane, hits lit as the engine fires them), BRIGHTNESS (O₃; a meter at the normalized value the lowpass takes, the hour's ppb beneath), DETUNE (PM2.5; ±σ in semitones and a band about the centre), REVERB (PM2.5; a meter at the normalized value the wet takes, the hour's µg/m³ beneath), and a ROUTING card on trial, laptop only: the three sources against the six destinations, a line per mapping brightening on the beat its source moved. At two columns its seven crossing lines read as a tangle, so below laptop width it is not shown; the source pills carry the routing there. Every card carries a source pill naming the measurement that drives it; the pill is the routing. Values move on the beat report; meters ease over a beat; at rest the cards show the held hour.
