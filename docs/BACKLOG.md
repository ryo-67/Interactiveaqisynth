...
| SCN-04 | P1 | PARTLY 2026-09-15 (D-43) | NO2 visual in the scene | The monitor's BEATS lane is the pulse on a panel (O-15's third option). The city band was dropped with the rebuild. Skyline, abstract ground band, or the pulse rendered on a panel (O-15) |
| SCN-05 | P1 | PARTIAL 2026-09-15 (fill alpha set by the worst-case arithmetic; measurement in a browser still to do) | Panel contrast measured against the brightest sky stop | Adaptive tones: dark fill 0.62 holds 4.9:1 for white text on a white sky; light fill holds dark text on a black sky. Verify by measurement on Jun 7 h14 and Oct 29 noon |
| GRA-02 | P1 | DONE 2026-09-15 (D-43, review pending) | A real visualization for the pulse row | The 16-step marks with per-bar hit counts are legible but carry no metaphor. Shoro: what the pulse should look like |
| DAT-13 | P2 | TODO | PM10 as a fifth graph tab, if AirNow's New York coverage supports it | AirNow reports O3, PM2.5, PM10, CO, NO2, SO2. New York's real-time PM10 was thin enough that the first prototype fabricated it (BUG-13). Needs a HourReading field, EPA parameter 81102 in the archive build, and the live route asking for it. Check coverage once AirNow is back |
...
| ID | Priority | Status | Task | Notes |
|---|---|---|---|---|
| A11Y-01 | P1 | PARKED 2026-09-17 | Landmarks and a page heading | The page has no `main`, `nav`, `header` or `footer` element and no `h1`, so a screen reader gets 26 correctly named controls and no structure to move between them. Everything else in the sweep was clean. BUG-48; the sky canvas wants `aria-hidden` with it, BUG-49 |
| CLN-09 | P2 | DONE 2026-09-17 | A linter, or drop the directives that pretend there is one | Measured (BUG-51): zero rules-of-hooks violations, and three unseen exhaustive-deps findings. The one worth acting on is useListenSession.ts:279, where `rest` is rebuilt every render and defeats the useMemo at 305 — the BUG-31 class. That one is fixed, along with the other two. ESLint is in as a devDependency with a deliberately narrow config — rules-of-hooks as an error, exhaustive-deps as a warning, unused directives reported — and `npm run lint`. Not a style pass: two rules, both about correctness. BUG-51 |
| CLN-13 | P2 | DONE 2026-09-17 | Sweep the remaining dependencies | After lucide-react, every declared package was checked against every file the repo owns. One more dead: `motion`, 772 KB in `dependencies`, where every `motion` in the source is the local token object from theme.ts (BUG-55). One missing: `typescript`, never declared and resolving transitively (BUG-56). The rest are all genuinely used, including the four `@types/*` and `tsx`, which runs the archive builders. Note for later: `tsx` is only invoked from a comment in scripts/build-archive.ts, so the builders have no npm script and are not discoverable from package.json |
| CLN-12 | P2 | DONE 2026-09-17 | Remove `lucide-react` | A production dependency nothing imports: 40 MB and 3,576 files on every install and every Vercel build, absent from the bundle entirely. CLN-03 kept it by mistake while icons.tsx had already inlined the paths and said so. BUG-54 |
| CLN-11 | P2 | DONE 2026-09-17 | Delete `src/components/RecordButton.tsx` and `src/utils/mockData.ts` | 272 lines with zero references, missed by the CLN-01 to CLN-05 sweep. Recording is on the backlog as POL-01, so the component is either wired up or deleted and rewritten when it is; keeping an unmounted one is the worst of both. BUG-52 |
| CLN-10 | P3 | TODO (accepted as harmless 2026-09-17) | Give SynthEngine a teardown | No dispose: Tone nodes, callbacks and the release-ramp interval live as long as the module, and a ramp in flight outlives the page. Harmless while the app is one page that never unmounts the engine; it becomes real the day a second engine is constructed. BUG-50 |
